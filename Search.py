...
class SearchTab(object):
    def init_search_tab(self):
        ...
        self.Bind(wx.EVT_BUTTON, self.export_search_results, id=xrc.XRCID('button:export_results'))
        ...
        tables = (
            ...
            'dbo.revisions',
            '-------------------------',
            'ecr.view_copper_variances',
            'ecr.view_total_copper_variance',
            '-------------------------',
            'tss.fsrs',
            'tss.rsrs',
            '-------------------------',
    ...
    def on_activated_result(self, event):
        ...
        if table_name == 'mmg.change_requests':
            sap_prod_ord = selected_item.GetItem(selected_item.GetFirstSelected(), 2).GetText()
    ...
    def export_search_results(self, event):
